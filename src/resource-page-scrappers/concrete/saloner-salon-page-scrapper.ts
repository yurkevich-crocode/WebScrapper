...
export class SalonerSalonPageScrapper extends SalonPageScrapper {
    
    async getSalonSchedule(): Promise<WorkHours[]> {
        const btnSelector = this.resourceCfg.selectors.showScheduleBtn;

        if (btnSelector) {
            await this.page.$eval(btnSelector, (node) => {
                //@ts-ignore
                node.click();
            });
        }
